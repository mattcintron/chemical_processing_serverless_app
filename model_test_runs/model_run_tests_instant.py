...
def process_chemical_data(data_list):
    ...
    # Post-process results to adjust non-chemical predictions
    for result in results:
        if result.get("prediction") == "Not a Chemical":
            # Retain only specified fields for non-chemical predictions
            result_keys = list(result.keys())
            for key in result_keys:
                if key not in {"row", "prediction", "Confidence score"}:
                    del result[key]
